...
const decisiveRows = new Set([
  "Advertised Price",
  "Real Build Cost",
  "Hidden Cost to Finish",
  "Completeness",
]);

function buildSections(kits: Kit[]) {
  const costs = kits.map((k) => k.trueCost);
  const minCost = Math.min(...costs);
  const maxCost = Math.max(...costs);
  const cheapestKit = kits[costs.indexOf(minCost)];
  const diff = maxCost - minCost;
  const pricingSummary = `${cheapestKit.brand} is $${diff.toLocaleString()} cheaper to build than the most expensive option.`;
  ...
  const specsSummary =
    bestStorage.slug === bestPanels.slug
      ? `${bestStorage.brand} leads in both solar wattage and battery storage.`
      : `${bestPanels.brand} has the most solar. ${bestStorage.brand} has the most storage.`;

  const complete = kits.filter((k) => k.completeness === 100);
  const componentsSummary =
    complete.length === kits.length
      ? "All kits include every required component."
      : complete.length > 0
        ? `Only ${complete.map((k) => k.brand).join(" and ")} include${complete.length === 1 ? "s" : ""} all components.`
        : "No kit is fully complete. All require additional component purchases.";
  ...
  return [
    ...
    },
    {
      ...
      rows: [
        { label: "Completeness", values: kits.map((k) => k.completeness), type: "number" as const, higherIsBetter: true, unit: "%" },
        { label: "Solar Panels", values: kits.map((k) => k.panelWatts), type: "number" as const, higherIsBetter: true, unit: "W" },
        { label: "Battery Storage", values: kits.map((k) => k.storageWh), type: "number" as const, higherIsBetter: true, unit: "Wh" },
        { label: "Inverter Output", values: kits.map((k) => k.inverterWatts), type: "number" as const, higherIsBetter: true, unit: "W" },
        { label: "System Voltage", values: kits.map((k) => `${k.voltage}V`), type: "text" as const },
        { label: "Chemistry", values: kits.map((k) => k.chemistry), type: "text" as const },
      ],
    },
    {
      ...
      rows: [
        { label: "Battery", values: kits.map((k) => k.included.battery), type: "boolean" as const },
        { label: "Inverter", values: kits.map((k) => k.included.inverter), type: "boolean" as const },
        { label: "Charge Controller", values: kits.map((k) => k.included.controller), type: "boolean" as const },
        { label: "Mounting", values: kits.map((k) => k.included.mounting), type: "boolean" as const },
        { label: "Monitoring", values: kits.map((k) => k.included.monitoring), type: "boolean" as const },
      ],
    },
  ];
}
...
export function CompareView({ allKits }: { allKits: Kit[] }) {
  const searchParams = useSearchParams();
  const kitSlugs = searchParams.get("kits");

  const compareKits = useMemo(() => {
    if (kitSlugs) {
    ...
    return [allKits[0], allKits[2], allKits[4]].filter(Boolean);
  }, [allKits, kitSlugs]);

  const verdicts = useMemo(() => computeVerdicts(compareKits), [compareKits]);
  const sections = useMemo(() => buildSections(compareKits), [compareKits]);

  if (compareKits.length < 2) {
    return (
      <div className="flex flex-col items-center justify-center py-16 rounded border border-dashed border-[var(--border)] bg-[var(--bg-surface)]">
        <p className="text-sm text-[var(--text-muted)] mb-2">Select at least 2 kits to compare</p>
        <Link href="/kits" className="text-xs text-[var(--accent)] hover:text-[var(--accent-hover)] transition-colors">
          &larr; Browse kits
        </Link>
      </div>
    );
          ...
          {sections.map((section) => (
            <div key={section.title}>
              ...
              {section.rows.map((row, ri) => (
                <div key={row.label} className={`grid grid-cols-4 gap-0 border-b border-[var(--border)] last:border-b-0 ${ri % 2 === 1 ? "bg-[var(--bg-primary)]/30" : ""}`}>
