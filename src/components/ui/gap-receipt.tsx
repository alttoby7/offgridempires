...
export function GapReceipt({ kit, hasVariantsWithPanels }: GapReceiptProps) {
  const receiptRef = useRef<HTMLDivElement>(null);
  const [shareState, setShareState] = useState<
    "idle" | "copied" | "downloaded" | "shared"
  >("idle");

  const includedValues = Object.values(kit.included);
  const isComponent = includedValues.length === 0;
  const hasAllIncluded = includedValues.length > 0 && includedValues.every(Boolean);
  const hasMissing = kit.missingCost > 0 || (!isComponent && !hasAllIncluded);
  ...
  // ── State: Single component (empty included map) ──────────────────────────
  if (isComponent) {
    return (
      <div className="rounded border border-[var(--border)] bg-[var(--bg-surface)] p-5">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-sm font-bold text-[var(--text-secondary)]">
            Single Component — Not a Full System
          </span>
        </div>
        <p className="text-xs text-[var(--text-muted)] ml-8">
          This is an individual component, not a complete solar kit.
          Additional parts are needed for a working system.
        </p>
      </div>
    );
  }
  ...
  // ── State: Incomplete — missing components not priced ─────────────────────
  if (requiredMissing.length === 0 && hasMissing) {
    const missingComponents = Object.entries(kit.included)
      .filter(([, v]) => !v)
      .map(([k]) => ROLE_LABELS[k] || k);

    return (
      <div className="rounded border border-[var(--accent)]/30 bg-[var(--accent)]/5 p-5">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-sm font-bold text-[var(--accent)]">
            Missing Components — Not Priced
          </span>
        </div>
        <div className="ml-8 space-y-2">
          <p className="text-xs text-[var(--text-secondary)]">
            This kit is missing {missingComponents.length} component{missingComponents.length > 1 ? "s" : ""} that
            {missingComponents.length > 1 ? " are" : " is"} not included in the advertised price:
          </p>
          <div className="flex flex-wrap gap-1.5">
            {missingComponents.map((name) => (
              <span
                key={name}
                className="inline-flex items-center gap-1 rounded-sm px-2 py-0.5 text-xs font-medium bg-[var(--accent)]/10 text-[var(--accent)] border border-[var(--accent)]/20"
              >
          ...
          {hasVariantsWithPanels && (
            <p className="text-xs text-[var(--accent)] font-medium mt-2">
              ↓ Panel-bundled configurations available below
            </p>
          )}
        </div>
      </div>
